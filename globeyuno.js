  // Track mouse position for tooltip placement (must be defined before listeners)
    ...
    if (typeof Globe === 'undefined') {
      ...
      world = new Globe(document.getElementById('globeViz'))
        ...
        .onZoom(({type}) => { /* noop to keep chain valid */ })
        ...
        // Use polygonLabel to show continent data on hover (for gap coverage)
        /*.polygonLabel(({ properties: d }) => {
          ...
          return `<div style="font-family: Inter, Arial, sans-serif; font-size: 12px; line-height: 1.4;">
            <div style="font-weight:600; color:#ffffff;">${continent}</div>
            <div style="color:#ffffff;">Country: ${d.ADMIN}</div>
          </div>`;
        })*/
        .hexPolygonColor(({ properties: d }) => {
          ...
          const colors = {
            base: '#B4BBF0',            // Default
            hoverContinent: '#B4BBF0',  // Continent hover
            hoverCountry: '#B4BBF0',    // Country hover (smooth blue)
      ...
      world.pointOfView({ lat: 45, lng: -100, altitude: 2.2 }, 0);
    ...
    function startAutoRotation() {
      ...
      autoRotateInterval = setInterval(() => {
        ...
        const newLng = (currentPOV.lng + 0.5) % 360;
        world.pointOfView({
          lat: currentPOV.lat,
